...
if __name__ == '__main__':
    ...
    class SEMSCASR(torch.nn.Module):
        def __init__(self, dim):
            super(SEMSCASR, self).__init__()
            # 自定义weight and bias
        ...
        def forward(self, pos, dim, input):
            w_c_i = torch.reshape(self.weight[pos], (2 * dim, dim))
            output = torch.tanh(torch.matmul(input, w_c_i) + self.bias)
            return output

    model = SEMSCASR(dim)

    # criterion = torch.nn.CrossEntropyLoss(weight=k, reduction='mean')
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)   # 不确定优化的对象参数对不对
    ...
    for epoch in range(total_epoch):
        ...
        for current_num, train_tup in enumerate(hownet.comp_train):
            ...
            phrase_vec = model(pos, dim, torch.cat((embed_word_whole, embed_sememe_whole), 1))
