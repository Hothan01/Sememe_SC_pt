...
if __name__ == '__main__':
    ...
    class SCASR(torch.nn.Module):
        def __init__(self, dim, trunc_num):
            super(SCASR, self).__init__()
            # 自定义内部需要更新的参数
        ...
        def forward(self, pos, dim, trunc_num, embed_whole):
            ...
            output = torch.tanh(torch.matmul(embed_whole, w_c_i + self.weight_W) + self.bias)
            return output

    model = SCASR(dim, trunc_num)
    ...
    for epoch in range(total_epoch):
        ...
        for current_num, train_tup in enumerate(hownet.comp_train):
            ...
            phrase_vec = model(pos, dim, trunc_num, torch.cat((embed_word_whole, embed_sememe_whole), 1))
            # 补充了L2正则化的loss
            L2_loss = labda * (torch.norm(model.weight_U, 2) + torch.norm(model.weight_V, 2) + torch.norm(model.weight_W, 2))
    ...
    for current_num, test_tup in enumerate(hownet.comp_test):
        if test_tup[4] in wordsim_words:
            ...
            phrase_vector = model(pos, dim, trunc_num, torch.cat((embed_word_whole, embed_sememe_whole), 1))
